  ...
  return (
    <div className="h-full overflow-y-auto p-5 lg:p-8 space-y-6 select-none bg-slate-50 dark:bg-[#040814] text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {missionStatus === "IN TRANSIT" || missionStatus === "MISSION ACTIVE" ? (
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 font-extrabold text-white text-xs lg:text-sm shadow-lg shadow-rose-600/40 transition animate-bounce cursor-pointer flex items-center gap-2 border border-rose-400/40"
            >
      ...
      {missionStatus === "ABORTED" && (
        <div className="rounded-2xl border border-rose-500/50 bg-rose-950/80 p-5 shadow-2xl space-y-1 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <XCircle className="h-6 w-6 text-rose-400 shrink-0" />
            <div>
              <h3 className="text-base font-black text-white">MISSION ABORTED BY OPERATOR</h3>
              <p className="text-xs lg:text-sm text-slate-300 font-medium">{abortReason || "Manual emergency abort sequence executed. UAV returning to origin base."}</p>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs lg:text-sm font-extrabold text-slate-200 hover:bg-slate-800"
          >
            Reset Flight System
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#070d1e] p-6 shadow-xl dark:shadow-2xl space-y-5 transition-colors duration-300">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-mono font-bold">MDoNER Direct Corridor</span>
            </div>

            <div className="space-y-4 text-xs lg:text-sm">
              {/* Origin Hub */}
              <div>
                <label className="text-slate-700 dark:text-slate-300 font-bold block mb-1.5">{t("uav.origin", "Origin Logistics Hub")}</label>
                <select
                  ...
                  className="w-full rounded-xl border border-slate-300 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3 text-slate-900 dark:text-white font-bold focus:border-sky-500 focus:outline-none"
                >
                  {hubs.map((h) => (
              ...
              {/* Destination Helipad */}
              <div>
                <label className="text-slate-700 dark:text-slate-300 font-bold block mb-1.5">{t("uav.destination", "Destination Helipad / Emergency LZ")}</label>
                <select
                  value={selectedLZ.id}
                  onChange={(e) => {
              ...
              {/* Assigned UAV Drone */}
              <div>
                <label className="text-slate-700 dark:text-slate-300 font-bold block mb-1.5">{t("uav.drone", "Assigned Lifeline UAV Drone")}</label>
                <select
              ...
              <div>
                <label className="text-slate-700 dark:text-slate-300 font-bold block mb-1.5">Emergency Payload Cargo</label>
                <input
                  type="text"
                  value={cargoName}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-slate-700 dark:text-slate-300 font-bold">{t("uav.payloadMass", "Payload Mass (kg)")}</span>
                  <span className={"font-black text-sm lg:text-base " + (isPayloadOverloaded ? "text-rose-600 dark:text-rose-400" : "text-sky-600 dark:text-sky-400")}>
            ...
            <button
              ...
              className={"w-full py-3.5 rounded-xl font-black text-xs lg:text-sm shadow-xl transition flex items-center justify-center gap-2 " + (
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#070d1e] p-6 shadow-xl dark:shadow-2xl space-y-3 transition-colors duration-300">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="text-lg lg:text-xl font-black uppercase text-slate-900 dark:text-white flex items-center gap-2.5">
                <Navigation className="h-5 w-5 text-sky-500 dark:text-sky-400" />
                {t("uav.aerialLifelineCorridor", "HIGH-ALTITUDE AERIAL LIFELINE CORRIDOR (ZERO ROAD DEPENDENCY)")}
              </h3>
              <span className="text-xs text-emerald-600 dark:text-emerald-400 font-mono font-bold">Leaflet GIS Vector</span>
            </div>

            <div className="relative rounded-xl overflow-hidden h-[360px] border border-slate-200 dark:border-slate-800 shadow-xl">
              <div ref={mapRef} className="w-full h-full z-0" />

              {/* Overlay HUD info badge */}
              <div className="absolute top-3 left-3 z-20 rounded-xl bg-white/90 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 p-3 text-xs font-mono backdrop-blur space-y-1 text-sky-600 dark:text-sky-400 font-bold">
