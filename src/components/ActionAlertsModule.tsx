...
export default function ActionAlertsModule({
  ...
  // Dynamic incident list with real-time broadcast SOS signals prepended
  const incidentList = [
    ...liveSosAlerts.map((sos: any) => ({
      id: sos.sosId || `SOS-${Math.floor(Math.random()*10000)}`,
      badge: "🚨 LIVE BEACON",
      badgeColor: "bg-rose-500/30 text-rose-300 border-rose-500 animate-pulse font-black",
      title: `Emergency Distress Beacon: ${sos.landmark || "NER Sector"} (${sos.distressType || "Disaster"})`,
      description: `GPS: ${sos.lat}° N, ${sos.lon}° E • Priority: ${sos.triageLevel || "Level 1"} • Trapped/Affected: ${sos.personsTrapped || "Unknown"}. Responders Alerted: ${sos.respondersNotified?.join(", ") || "NDRF, SDRF"}.`,
      gps: `${sos.lat}° N, ${sos.lon}° E`,
      dispatched: sos.respondersNotified?.[0] || "NDRF Fast-Response Unit",
      timeAgo: "Just now (Live Broadcast)",
      actionType: "reroute",
      actionLabel: "Track SOS 📍"
    })),
    ...defaultIncidentList
  ];

  const latestSOS = liveSosAlerts[0];
                ...
                <div className="flex items-center gap-2 shrink-0 self-end lg:self-center">
                  {inc.actionType === "reroute" && (
                    <button
                      ...
                      className="rounded-xl bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white font-black text-xs px-4 py-2 shadow-md transition cursor-pointer"
                    >
                      {inc.actionLabel}
                    </button>
                  )}
                  ...
                  {inc.actionType === "track" && (
                    <button
                      onClick={onNavigateToMap}
                      className="rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs px-4 py-2 shadow-md transition cursor-pointer"
                    >
                      {inc.actionLabel}
                    </button>
                  )}
