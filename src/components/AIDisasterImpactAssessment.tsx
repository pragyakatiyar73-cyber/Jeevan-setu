 ...
 onNavigateToMonitoring, initialLoc }: AIDisasterImpactAssessmentProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#030712] text-slate-100 p-4 sm:p-6 space-y-6 font-sans">
      ...
      <div className="rounded-2xl border border-slate-800 bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 p-5 shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isDemoMode && (
              <span className="rounded bg-amber-500/20 px-2.5 py-0.5 text-xs font-bold text-amber-400 border border-amber-500/40">
                ⚠️ DEMO DATA ACTIVE
              </span>
            )}
          </div>

          <h1 className="mt-2 text-2xl font-black text-white tracking-tight flex items-center gap-2">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            className={`rounded-xl border px-3.5 py-2 text-xs font-bold flex items-center gap-2 transition ${
              isDemoMode ? 'bg-amber-500/20 text-amber-400 border-amber-500/40' : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-white'
            }`}
          >
            {isDemoMode ? <ToggleRight className="h-4 w-4 text-amber-400" /> : <ToggleLeft className="h-4 w-4 text-slate-500" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span className="text-[10px] text-slate-400 font-mono">JPG, PNG, WEBP &bull; Max 10MB</span>
          </div>

          {/* Upload Dropzone */}
          <div className="relative h-64 w-full rounded-xl overflow-hidden border-2 border-dashed border-slate-700 bg-slate-950 flex flex-col items-center justify-center p-4">
            {selectedImage ? (
              <>
                <img src={selectedImage} alt="Disaster Evidence Preview" className="h-full w-full object-cover rounded-lg" />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent p-3 flex justify-between items-end">
                  <div className="text-xs">
                    <div className="font-bold text-white truncate max-w-[200px]">{fileName}</div>
                    <div className="text-[10px] text-slate-400 font-mono">{fileSizeStr} &bull; Uploaded {uploadTime}</div>
                  </div>
                  <button
              ...
              <label className="cursor-pointer flex flex-col items-center justify-center space-y-2 text-center p-6">
                <Upload className="h-10 w-10 text-indigo-400 animate-pulse" />
                <span className="text-xs font-bold text-slate-200">{t("aiimpact.uploadPhotoSub", "Click or Drag & Drop disaster site image here")}</span>
                <span className="text-[11px] text-slate-500">{t("aiimpact.supportsText", "Supports ground photos, mobile captures & drone imagery")}</span>
                <input
                  type="file"
          ...
          {uploadError && (
            <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-xs text-rose-400 flex items-center gap-2">
          ...
          <div className="space-y-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Pre-Loaded NER Disaster Imagery Presets</span>
            <div className="grid grid-cols-3 gap-2">
              {sampleImages.map((sample, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSelectSample(sample)}
                  className="group relative h-20 rounded-xl overflow-hidden border border-slate-800 hover:border-indigo-500 transition text-left"
                >
        ...
        <div className="lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-400 font-mono">Geocoded Context</span>
            </div>
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="relative">
                <form onSubmit={handleLocationSearch} className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-700 bg-slate-950 pl-9 pr-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none"
                    />
                  </div>
                  <button
                    type="submit"
                ...
                {searchResults.length > 0 && (
                  <div className="absolute left-0 right-0 top-12 z-[2000] rounded-xl border border-slate-800 bg-slate-950 p-2 shadow-2xl max-h-48 overflow-y-auto">
              ...
              <div className="flex items-center gap-2">
                <form onSubmit={handleManualCoordSubmit} className="flex items-center gap-2 flex-1">
                  ...
                  <button type="submit" className="rounded-lg border border-slate-700 bg-slate-800 px-2.5 py-1.5 text-xs text-slate-200 hover:bg-slate-700">
                    Set
                  </button>
                </form>
                ...
                <button
                  ...
                  className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/20 flex items-center gap-1"
                >
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs">
                ...
                <div className="font-bold text-white mt-0.5">{locationLoc.displayName}</div>
                <div className="text-[11px] text-slate-400 font-mono mt-1">
                  Latitude: <b className="text-slate-200">{locationLoc.lat.toFixed(4)}° N</b> &bull; Longitude: <b className="text-slate-200">{locationLoc.lon.toFixed(4)}° E</b>
                </div>
              </div>

              {/* Leaflet Pin Map Picker */}
              <div className="h-36 w-full rounded-xl overflow-hidden border border-slate-800 relative">
                <div ref={mapContainerRef} className="h-full w-full" />
                <div className="absolute left-2 bottom-2 z-[1000] rounded-lg border border-slate-800 bg-slate-950/90 px-2 py-1 text-[10px] text-slate-300 backdrop-blur">
          ...
          <button
            ...
            className={`w-full mt-3 rounded-xl py-3.5 text-xs font-bold text-white shadow-xl flex items-center justify-center gap-2 transition ${
              !selectedImage || isAnalyzing ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-600 via-sky-600 to-rose-600 hover:from-indigo-500 hover:to-rose-500 shadow-indigo-600/30'
            }`}
          >
            {isAnalyzing ? (
              <>
      ...
      {assessment && (
        <div className="space-y-6">
          ...
          <div className={`rounded-2xl border p-6 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 ${
            assessment.overallSeverity === 'CRITICAL' ? 'border-rose-500/50 bg-gradient-to-r from-slate-900 via-rose-950/40 to-slate-900' :
            assessment.overallSeverity === 'HIGH' ? 'border-amber-500/50 bg-gradient-to-r from-slate-900 via-amber-950/40 to-slate-900' :
            'border-sky-500/50 bg-gradient-to-r from-slate-900 via-sky-950/40 to-slate-900'
          }`}>
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">DISASTER IMPACT SEVERITY</span>
                <span className="rounded bg-slate-950/80 px-2 py-0.5 text-[10px] font-mono text-slate-300 border border-slate-800">
                  {assessment.disclaimer}
                </span>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-3xl font-black tracking-tight text-white flex items-center gap-2">
                  <ShieldAlert className={`h-8 w-8 ${assessment.overallSeverity === 'CRITICAL' ? 'text-rose-500' : 'text-amber-400'}`} />
            ...
            <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-6">
              <div className="text-right">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">AI-Assisted Estimated Impact Score</span>
                <div className="text-3xl font-black text-indigo-400">{assessment.impactScore} <span className="text-base text-slate-400 font-normal">/ 100</span></div>
                <div className="text-[11px] text-emerald-400 font-semibold font-mono">Confidence: {assessment.confidencePercent}%</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                ...
                <span className="text-[10px] text-emerald-400 font-mono font-bold">CONFIDENCE: {assessment.confidencePercent}%</span>
              </div>

              <div className="space-y-3 text-xs">
                <div className="flex justify-between p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                  <span className="text-slate-400">Classified Disaster Type:</span>
                  <span className="font-bold text-white">{assessment.vision.disasterType}</span>
                </div>
                ...
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {assessment.vision.affectedInfrastructure.map((infra, idx) => (
                      <span key={idx} className="rounded bg-indigo-500/20 px-2 py-0.5 text-[11px] font-bold text-indigo-300 border border-indigo-500/30">
                        {infra}
                      </span>
                    ))}
                  </div>
            ...
            <div className="lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                ...
                <span className="text-[10px] text-slate-400">Human & Infra Risk</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1">
                  ...
                  <div className={`text-base font-bold ${assessment.impactCategories.humanSafety.rating === 'CRITICAL' ? 'text-rose-500' : 'text-amber-400'}`}>
                    {assessment.impactCategories.humanSafety.rating}
                  </div>
                  <p className="text-[10px] text-slate-400 leading-snug">{assessment.impactCategories.humanSafety.evidence}</p>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1">
                  <span className="text-[10px] text-slate-500 uppercase font-bold">Infrastructure Impact</span>
                  <div className={`text-base font-bold ${assessment.impactCategories.infrastructure.rating === 'CRITICAL' ? 'text-rose-500' : 'text-amber-400'}`}>
                    {assessment.impactCategories.infrastructure.rating}
                  </div>
                  <p className="text-[10px] text-slate-400 leading-snug">{assessment.impactCategories.infrastructure.evidence}</p>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1">
                  <span className="text-[10px] text-slate-500 uppercase font-bold">Transportation Impact</span>
                  <div className={`text-base font-bold ${assessment.impactCategories.transportation.rating === 'CRITICAL' ? 'text-rose-500' : 'text-amber-400'}`}>
                    {assessment.impactCategories.transportation.rating}
                  </div>
                  <p className="text-[10px] text-slate-400 leading-snug">{assessment.impactCategories.transportation.evidence}</p>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1">
                  <span className="text-[10px] text-slate-500 uppercase font-bold">Environmental Impact</span>
                  <div className={`text-base font-bold ${assessment.impactCategories.environmental.rating === 'CRITICAL' ? 'text-rose-500' : 'text-indigo-300'}`}>
                    {assessment.impactCategories.environmental.rating}
                  </div>
                  <p className="text-[10px] text-slate-400 leading-snug">{assessment.impactCategories.environmental.evidence}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3 text-xs">
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  ...
                  <div className="text-lg font-bold text-white">{assessment.environmentalContext.temperature}°C</div>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Precipitation</span>
                  <div className="text-lg font-bold text-sky-400">{assessment.environmentalContext.precipitation} mm</div>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Terrain Slope</span>
                  <div className="text-lg font-bold text-amber-400">{assessment.environmentalContext.slopeDegrees}°</div>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Soil Moisture</span>
                  <div className="text-lg font-bold text-indigo-400">{assessment.environmentalContext.soilMoisturePercent}%</div>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Fault Line Dist</span>
                  <div className="text-lg font-bold text-slate-300">{assessment.environmentalContext.faultDistanceKm} km</div>
                </div>

                <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Elevation</span>
                  <div className="text-lg font-bold text-emerald-400">{assessment.environmentalContext.elevationMsl}m</div>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    {assessment.riskFactors.map(rf => (
                      <div key={rf.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                        <span className="font-semibold text-white flex items-center gap-2">
                          <span className={`h-2 w-2 rounded-full ${rf.severity === 'RED' ? 'bg-rose-500' : 'bg-amber-400'}`}></span>
                          {rf.factor}
                        </span>
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-4 flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-xs text-slate-400">Generated at {assessment.timestamp} &bull; Target: {assessment.locationName}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {onNavigateToMonitoring && (
                  <button
                    ...
                    className="rounded-xl border border-indigo-500/40 bg-indigo-500/10 px-4 py-2.5 text-xs font-bold text-indigo-300 hover:bg-indigo-500/20 flex items-center gap-2"
                  >
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950 p-6 space-y-4 text-xs">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-b border-slate-800 pb-4">
                <div>
                  ...
                  <span className="text-white font-bold text-sm">{assessment.disasterType}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-bold block">Overall Severity</span>
                  <span className="text-rose-400 font-bold text-sm">{assessment.overallSeverity}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-bold block">Impact Score</span>
                  <span className="text-indigo-400 font-bold text-sm">{assessment.impactScore} / 100</span>
                </div>
                <div>
                  <span className="text-slate-500 font-bold block">Coordinates</span>
                  <span className="text-slate-300 font-mono text-xs">{assessment.lat.toFixed(4)}°N, {assessment.lon.toFixed(4)}°E</span>
                </div>
              </div>

              <div className="space-y-2">
                <span className="font-bold text-slate-300 block uppercase tracking-wider">Executive Findings Summary</span>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-[11px] text-slate-400 flex items-start gap-2">
        ...
        <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setMdonerModalOpen(false)} className="text-slate-400 hover:text-white font-bold">✕</button>
            </div>

            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-400 block mb-1 font-semibold">Target Agency</label>
                <input
                  type="text"
                  value={mdonerAgency}
              ...
              <div>
                <label className="text-slate-400 block mb-1 font-semibold">Disaster Location & Severity</label>
              ...
              <div>
                <label className="text-slate-400 block mb-1 font-semibold">Emergency Dispatch Alert Message</label>
                <textarea
                  rows={3}
                  value={mdonerMsg}
